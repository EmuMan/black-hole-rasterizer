...
bool WindowManager::init(int const width, int const height)
{
	...
	windowHandle = glfwCreateWindow(width, height, "Final Project - Jacob Kelleran", nullptr, nullptr);
	if (! windowHandle)
	{
		glfwTerminate();
		return false;
	...
	glfwSetCursorPosCallback(windowHandle, mousePos_callback);
...
void WindowManager::mousePos_callback(GLFWwindow* window, double in_newX, double in_newY)
{
	if (instance && instance->callbacks)
	{
		instance->callbacks->mousePosCallback(window, in_newX, in_newY);
	}
}
